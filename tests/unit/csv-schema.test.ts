...
describe('CsvRowSchema', () => {
  it('parses a fully valid row', () => {
    ...
    expect(result.data.entrance_requirements).toEqual({ turkey: { yos: true } });
  });
  ...
  it('accepts moe_approved = "false" and converts to boolean false', () => {
    const result = CsvRowSchema.safeParse({ ...validRow, moe_approved: 'false' });
    expect(result.success).toBe(true);
    if (result.success) expect(result.data.moe_approved).toBe(false);
  });

  it('accepts moe_approved case-insensitively (TRUE)', () => {
    const result = CsvRowSchema.safeParse({ ...validRow, moe_approved: 'TRUE' });
    expect(result.success).toBe(true);
  ...
  it('treats absent ranking_qs as null', () => {
    const { ranking_qs: _, ...rowWithout } = validRow;
    const result = CsvRowSchema.safeParse(rowWithout);
    expect(result.success).toBe(true);
    if (result.success) expect(result.data.ranking_qs).toBeNull();
  });
  ...
  it('splits pipe-separated languages and trims whitespace', () => {
    const result = CsvRowSchema.safeParse({ ...validRow, languages: 'English | Turkish | German' });
    expect(result.success).toBe(true);
    if (result.success) expect(result.data.languages).toEqual(['English', 'Turkish', 'German']);
  });

  it('splits pipe-separated majors', () => {
    const result = CsvRowSchema.safeParse({ ...validRow, majors: 'CS|Law|Medicine' });
    expect(result.success).toBe(true);
    if (result.success) expect(result.data.majors).toEqual(['CS', 'Law', 'Medicine']);
  });

  it('fails when official_website is not a valid URL', () => {
    const result = CsvRowSchema.safeParse({ ...validRow, official_website: 'not-a-url' });
    expect(result.success).toBe(false);
  });

  it('fails when application_portal_url is not a valid URL', () => {
    const result = CsvRowSchema.safeParse({ ...validRow, application_portal_url: 'just-text' });
    expect(result.success).toBe(false);
  });

  it('fails when entrance_requirements is invalid JSON', () => {
    const result = CsvRowSchema.safeParse({ ...validRow, entrance_requirements: '{invalid}' });
    expect(result.success).toBe(false);
  });

  it('treats blank entrance_requirements as empty object', () => {
    const result = CsvRowSchema.safeParse({ ...validRow, entrance_requirements: '' });
    expect(result.success).toBe(true);
    if (result.success) expect(result.data.entrance_requirements).toEqual({});
  });

  it('treats absent entrance_requirements as empty object', () => {
    const { entrance_requirements: _, ...rowWithout } = validRow;
    const result = CsvRowSchema.safeParse(rowWithout);
    expect(result.success).toBe(true);
    if (result.success) expect(result.data.entrance_requirements).toEqual({});
  });
